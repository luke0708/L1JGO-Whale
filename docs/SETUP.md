...
伺服器需要從客戶端 `.s32` 地圖檔解析後的碰撞資料（`.txt` 格式），用於碰撞偵測和移動驗證。

1. 使用地圖解析工具將客戶端的 `.s32` 檔轉換為 `.txt` 格式
2. 將轉換後的檔案放到專案根目錄的 `map/` 資料夾
3. 檔案命名格式：`<mapID>.txt`（例如 `2005.txt`）

> **注意**：`.s32` 是客戶端原始的地圖二進制格式，伺服器不直接使用。沒有對應地圖資料的地圖，NPC 移動和碰撞偵測將無法正常運作，但玩家仍可登入。
